...
set all_projects=Validate.Fluently
rem Setup Validate Fluently
set fluently_projects=Validate.Fluently
...
if "%1" == "--fluent" (
    if "%projects%" == "" (
        set projects=%fluently_projects%
    ) else (
        set projects=%projects%%delim%%fluently_projects%
    )
    goto :fini_arg
)
if "%1" == "--fluently" (
